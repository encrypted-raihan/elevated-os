...
function startUserListener() {
  ...
  const currentRole = normalizeRole(state.profile.role);
  ...
  state.userUnsub = onSnapshot(
    ...
    (snapshot) => {
      ...
      state.users = allUsers.filter((u) => shouldShowUser(u, uid, currentRole));
      renderConversationList();
...
function normalizeFilter(filter) {
  ...
  if (["unread", "team", "admins", "archived", "all"].includes(value)) return value;
  return "all";
}
...
function buildEligibleContacts() {
  ...
  return state.users
    .filter((user) => shouldShowUser(user, currentUid, currentRole))
    .map((user) => {
    ...
    .sort((a, b) => {
      const aRank = a.badge === "admin" ? 0 : 1;
      const bRank = b.badge === "admin" ? 0 : 1;
...
function getVisibleContacts() {
  ...
  return contacts
    .filter((contact) => {
      ...
      if (state.filter === "admins" && contact.badge !== "admin") return false;
      if (state.filter === "team" && contact.badge === "admin") return false;
...
function formatContactType(contact) {
  return contact.badge === "admin" ? "Admin" : "Team";
}

function tryOpenPendingConversation() {
  if (!state.pendingOpenId) return;
  if (!state.ready) return;

  const contact = buildEligibleContacts().find((item) => item.id === state.pendingOpenId);
  if (!contact) return;

  state.pendingOpenId = null;
  openConversation(contact.id, { markRead: true });
}
...
function renderConversationHeader() {
  ...
  if (!conv) {
    ...
    els.archiveBtn.textContent = "Archive Chat";
    return;
  }
